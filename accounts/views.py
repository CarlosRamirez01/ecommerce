...
def register(request):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            user.save()
            # Envio de correo personalizado
            current_site = get_current_site(request)
            mail_subject = 'Activa tu cuenta ahora!'
            ...
            send_email.send()
            # Fin de envio de correo personalizado
...
def login(request):
    if request.method == 'POST':
        ...
        if user is not None:
            ...
            messages.success(request, 'Se ha iniciado sesión exitosamente!')
...
def forgotPassword(request):
    if request.method == 'POST':
        ...
        if Account.objects.filter(email=email).exists():
            ...
            messages.success(request, 'Se ha enviado un email al correo electronico inscrito, por favor verifica.')
            return redirect('login')
        else:
...
def resetPassword(request):
    if request.method == 'POST':
        ...
        if password == confirm_password:
            ...
            user.save()
            messages.success(request, 'Se ha restablecido la contraseña correctamente!')
            return redirect('login')
        else:
            messages.erro(request, 'No concuerdan las contraseñas suministradas!')
            return redirect('resetPassword')
    else:
